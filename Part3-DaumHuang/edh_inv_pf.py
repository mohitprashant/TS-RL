...
class PFPF_EDHFilter(ParticleFlow):
    """
    Invertible Particle Flow Particle Filter (PF-PF) with EDH Flow.
    """
    ...
    def run_step(self, particles, weights, m_ekf, P_ekf, y_curr):
        """
        Executes one time step of the PF-PF (EDH) filter.
        ...
        particles_prop = self.model.transition(particles)                                    # Migrate particles
        ...
        for i in range(self.steps):
            ...
            x_bar = tf.reduce_sum(particles_prop * weights[:, None], axis=0)                # Linearize at Weighted Mean
            ...
            det = tf.linalg.det(tf.eye(self.model.K) + dlam * A)                            # Accumulate Jacobian Determinant: det(I + eps*A)
        ...
        w_unnorm = tf.exp(log_w - tf.reduce_max(log_w))                                      # Normalize
